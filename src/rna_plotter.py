...
def load_rna_plotter():
    pileup = pd.read_hdf('data/rna_seq_pileup.h5.z')
    rna_plotter = RNASeqPlotter(pileup)
    return rna_plotter
...
class RNASeqPlotter:
    ...
    def plot(self, ax=None, time=None, flip_genome=False):
        """Plot RNA-seq"""
        ...
        for i in range(len(times)):
            ...
            if len(data) > 0:
                _plot_pileup(ax, data, self.blues[color_ind], '+')

            data = time_pileup[(time_pileup.strand == '-')]
            if len(data) > 0:
                _plot_pileup(ax, data, self.reds[color_ind], '-')
        ...
        if flip_genome:
            ax.set_xlim(self.span[1], self.span[0])
            ax.set_ylim(16, -16)
        else:
...
def get_strand_colors(num_times=6):
    ...
    blues = list(map(lambda x: cm.Blues(float(x)/(N), 1), range(color_offset, N)))
    reds = list(map(lambda x: cm.Reds(float(x)/(N), 1), range(color_offset, N)))
    return reds, blues
